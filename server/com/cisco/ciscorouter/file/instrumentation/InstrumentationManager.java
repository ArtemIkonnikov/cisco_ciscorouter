package com.cisco.ciscorouter.file.instrumentation;
...
import com.harris.netboss.server.shared.dda.driver.snmp.AbstractDriverSnmpAPI;
...
import com.harris.netboss.server.shared.instrumentation.DefaultIndicationHandler;
import com.harris.netboss.server.shared.instrumentation.DefaultInstrumentationManager;
import com.harris.netboss.server.shared.instrumentation.ExtendedSnmpHandler;
...
public class InstrumentationManager extends DefaultInstrumentationManager
	...
	private class AlarmHandler extends DefaultAlarmHandler {
		...
		public void pollAllAlarms() {
			try {
				alarmPollingInProgress = true;
				if (systemHandler.checkSystem()) {
					super.pollAllAlarms();
					activeAlertIds = getIndicationHandler()
							.getAllActiveAlertIds();
					final Object result = executeFunction("Alarm",
							new Object[0]);
					if (result != null) {
						reconcileAlarms();
					}
					synchronized (activeAlertIdsLock) {
	...
	protected final Map<String, Long> allInventory = new ConcurrentHashMap<String, Long>();
	protected final Map<String, Long> allInventoryCopy = new ConcurrentHashMap<String, Long>();

	private final Object activeAlertIdsLock = new Object();
	private Boolean alarmPollingInProgress = false;
	protected Set<Long> activeAlertIds;
	...
	public Map<String, Long> getAllInventory() {
		String compFunc = "Instrumentationmanager:::getAllInventory()";
		synchronized (allInventory) {
			allInventory.clear();
			if (resyncClasses.isEmpty()) {
				allInventory.putAll(this
						.getInventory("UIM_ManagedSystemElement"));
			} else {
	...
	public void reconcileAlarms() {
		...
		synchronized (activeAlertIdsLock) {
			logger.log(
					"Clearing "
							+ (activeAlertIds != null ? activeAlertIds.size()
									: 0) + " alerts", classMethod,
					AgentLogger.INFO);
			if (activeAlertIds != null) {
				for (final Long activeAlertId : activeAlertIds) {
					if (!shouldRun(classMethod)) {
						return;
					}
					getIndicationHandler().clearAlert(activeAlertId);
				}
			}
		}
	}
	...
	public void sendAlarm(final HashMap<String, Object> indHash) {
				...
				if (!name.isEmpty()) {
					...
					if (instID == null) {
						logger.log(
								...
								classMethod, AgentLogger.WARN);
						String ameExtension = (String) indHash.get(AlertPropertyNameConstants.ALERTING_MANAGED_ELEMENT_NAME_EXTENSION);
						if (ameExtension != null && !ameExtension.isEmpty()) {
							indHash.put(
		...
		if (alarmPollingInProgress
				&& (Integer) indHash.get(AlertPropertyNameConstants.STATE) == 1) {
			if (getIndicationHandler() instanceof DefaultIndicationHandler) {
				((DefaultIndicationHandler)getIndicationHandler()).validateAlertIndication(indHash);
			}
			Long alertId = getIndicationHandler().activateAlert(indHash);
			synchronized (activeAlertIdsLock) {
				if (activeAlertIds != null && alertId != null) {
			...
			getIndicationHandler().sendIndication(indHash);
		}
	}
	...
	private void sendSystemAlarm(final int type, final String name,
		...
		final Map<String, Object> indHash = new HashMap<String, Object>();
		indHash.put(AlertPropertyNameConstants.DISPLAY_NAME, name);
		indHash.put(
				AlertPropertyNameConstants.CIM_ALERTING_MANAGED_ELEMENT_ID,
		...
		indHash.put(AlertPropertyNameConstants.PROBABLE_CAUSE, probCause);
		indHash.put(AlertPropertyNameConstants.DESCRIPTION, description);
		indHash.put(AlertPropertyNameConstants.CAPTION, name);
		indHash.put(AlertPropertyNameConstants.STATE, (raise ? 1 : 3));
		indHash.put(AlertPropertyNameConstants.ALERT_TYPE, type);
		indHash.put(AlertPropertyNameConstants.SPECIFIC_CAUSE, message);
		indHash.put(AlertPropertyNameConstants.PERCEIVED_SEVERITY,
				severity);
		if (getIndicationHandler() instanceof DefaultIndicationHandler) {
			((DefaultIndicationHandler)getIndicationHandler()).validateAlertIndication(indHash);
		}
	...
	public Map<String, Object> snmpWalkByIP(final String singleOid,
			final String ipAddress) {
		return snmpWalkByIPAndCommunity(singleOid, ipAddress, "");
	}

	public Map<String, Object> snmpWalkByIPAndCommunity(final String singleOid,
			final String ipAddress, final String snmpV2CommunityString) {
		if (getSnmpHandler() != null
				&& getSnmpHandler() instanceof ExtendedSnmpHandler) {
			final Map<String, Object> result = new TreeMap<String, Object>(
					new Comparator<String>() {
						@Override
						public int compare(String o1, String o2) {
			...
			result.putAll(((ExtendedSnmpHandler) getSnmpHandler()).snmpWalk(
					singleOid, ipAddress, snmpV2CommunityString));
			return result;
		}
		return null;
	}

	public Object snmpGetByIP(final String oid, final String ipAddress) {
		return snmpGetByIPAndCommunity(oid, ipAddress, "");
	}

	public Object snmpGetByIPAndCommunity(final String oid,
			final String ipAddress, final String snmpV2CommunityString) {
		if (getSnmpHandler() != null
				&& getSnmpHandler() instanceof ExtendedSnmpHandler) {
			...
			final ArrayList<String> oidList = new ArrayList<String>();
			oidList.add(oid);
			final Map<String, Object> temp = ((ExtendedSnmpHandler) getSnmpHandler())
					.snmpGet(oidList, ipAddress, snmpV2CommunityString);
			for (final String key : temp.keySet()) {
				return temp.get(key);
			}
		}
	...
	public Object snmpGetByV3ContextNameAndSecurityName(final String singleOid,
			final String snmpv3ContextName, final String snmpv3SecurityName) {
		if (getSnmpHandler() != null
				&& getSnmpHandler() instanceof ExtendedSnmpHandler) {
			final ArrayList<String> oidList = new ArrayList<String>();
			oidList.add(singleOid);
			Map<String, Object> connProps = new HashMap<String, Object>();
			connProps
					.put(AbstractDriverSnmpAPI.SNMP_CONNECTION_PROPERTIES.SNMP_CONTEXT_NAME,
							snmpv3ContextName);
			connProps
					.put(AbstractDriverSnmpAPI.SNMP_CONNECTION_PROPERTIES.SNMP_SECURITY_NAME,
							snmpv3SecurityName);
			final Map<String, Object> temp = ((ExtendedSnmpHandler) getSnmpHandler())
					.snmpGet(oidList, false, connProps);
			for (final String key : temp.keySet()) {
				return temp.get(key);
			}
		}
	...
	public Map<String, Object> snmpWalkByV3ContextNameAndSecurityName(
			final String singleOid, final String snmpv3ContextName,
			final String snmpv3SecurityName) {
		if (getSnmpHandler() != null
				&& getSnmpHandler() instanceof ExtendedSnmpHandler) {
			Map<String, Object> connProps = new HashMap<String, Object>();
			connProps
